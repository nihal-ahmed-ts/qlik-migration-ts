...
The canonical Qlik→ThoughtSpot formula mapping — **199 rows** across 17 categories. This is the human-readable rendering; the machine-readable source that `q2t formulas` loads is `tools/q2t-cli/q2t/data/qlik_ts_formula_map.json` (regenerate with `build_formula_map.py`).

Both `ts-convert-from-qlik` and `ts-convert-from-qlik-api` consult this reference before declaring any Qlik expression untranslatable.
...
| S02 | `Upper(str)` | `upper(col)` | ok | Direct equivalent. Converts all characters to uppercase. |
| S03 | `Lower(str)` | `lower(col)` | ok | Direct equivalent. Converts all characters to lowercase. |
