<!-- currency: qlik — 2026-07 (q2t IR v0.1) -->

# Qlik App IR — the extract↔transform contract

The **intermediate representation (IR)** is the single contract between the
Qlik-extraction side and the ThoughtSpot-transform side of `q2t`. Extractors
(offline, engine, engine-artifacts, qlik-cloud) populate these structures;
`q2t transform` consumes them. The IR is plain JSON — dump it, inspect it,
hand-edit it, and re-run later stages without touching Qlik again.

Source of truth: `tools/q2t-cli/q2t/ir.py`. Every field is optional-friendly —
